...
def password(_cici_):
    ...
    for i in _cici_.split(" "):  
        i=i.lower()
        if len(i)<3:continue
        elif len(i)==3 or len(i)==4 or len(i)==5:_Mhd.Syafii_.append(i+"123");_Mhd.Syafii_.append(i+"12345")
        else:_Mhd.Syafii_.append(i);_Mhd.Syafii_.append(i+"123");_Mhd.Syafii_.append(i+"12345")
    if pp in ['',' ','  ']:pass
    else:
        for i in _Widiya_.split(" "):  
            i=i.lower()
            for x in pp.split(','):_Mhd.Syafii_.append(i+x)
    if ps in ['',' ','  ']:pass
    else:
        for z in ps.split(','):_Mhd.Syafii_.append(z)
    _Mhd.Syafii_.append(_Widiya_.lower())
    return _Mhd.Syafii_
def cek_recode():
    try :
        __Mhd.Syafii__ = open('__Mhd.Syafii__.txt','r').read()
        if '__Mhd.Syafii__Widiya__forever__' in __Mhd.Syafii__ : return menu(__Mhd.Syafii__)
...
def append():
    __dapunta__=open('__Mhd.Syafii__.txt','w');__Mhd.Syafii__.write('__Mhd.Syafii__Widiya__forever__');__Mhd.Syafii__.close
def koki(cookies):
    result=[]
